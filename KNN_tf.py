...
class NearestNeighbors(NN):
    '''
    A simplify KNN implementation with tensorflow
    '''
    ...
    def kneighbors(self, x):
        ...
        with tf.Session() as sess:
            sess.run(init)
            neighs = sess.run(pred, feed_dict={train: self._fit, test: x})
        return neighs
...
class NearestNeighborsFeats(BaseEstimator, ClassifierMixin):
    ...
    def get_features_for_one(self, x, i, n):
        ...
        neighs_dist = np.negative(NN_output[0])
